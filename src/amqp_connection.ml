...
let open_connection { framing; virtual_host; _ } =
  let open Amqp_spec.Connection in
...
let connect ?(virtual_host="/") ?(port=5672) ?(credentials=("guest", "guest")) ~host () =
  ...
  Amqp_spec.Connection.Start.reply (framing, 0) (handle_start credentials) >>= fun () ->
  Amqp_spec.Connection.Tune.reply (framing, 0) handle_tune >>= fun () ->
  open_connection t >>= fun () ->

  (* We cannot wait for connection close *)
  don't_wait_for (Amqp_spec.Connection.Close.reply (framing, 0) handle_close);
  return t
